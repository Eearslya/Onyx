...
// Chain GLFW callbacks for main viewport: our callbacks will call the user's previously installed
// callbacks, if any.
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons() {
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n++) {
    ...
    // If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the back-end, imgui will ignore
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport) {
  ...
  data->Window = glfwCreateWindow((int)viewport->Size.x, (int)viewport->Size.y, "No Title Yet",
                                  NULL, share_window);
...
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (msg == WM_NCHITTEST) {
    ...
    // io.MouseHoveredViewport properly (which is OPTIONAL). The ImGuiViewportFlags_NoInputs flag is
    // set while dragging a viewport, as want to detect the window behind the one we are dragging.
    // If you cannot easily access those viewport flags from your windowing/event code: you may
    // manually synchronize its state e.g. in your main loop after calling UpdatePlatformWindows().
