...
namespace Onyx {
namespace Vulkan {
const std::vector<std::string> VulkanDevice::_requiredExtensions = {
...
VulkanDevice::VulkanDevice(VkInstance instance, const bool validationEnabled,
  ...
  if (!SelectPhysicalDevice()) {
    Logger::Fatal("Unable to find a suitable Vulkan device!");
  }
}

VulkanDevice::~VulkanDevice() {}

const bool VulkanDevice::SelectPhysicalDevice() {
  ...
  for (U32 i = 0; i < physicalDeviceCount; i++) {
    QueryDeviceDetails(physicalDevices[i], physicalDeviceDetails[i]);
#ifdef ONYX_TRACE
  ...
  _physicalDevice = candidateDevices[0]->Device;
  _physicalDeviceDetails = VulkanPhysicalDeviceDetails(*candidateDevices[0]);
...
void VulkanDevice::QueryDeviceDetails(VkPhysicalDevice physicalDevice,
  ...
  QueryDeviceQueues(physicalDevice, details.Queues);
  QueryDeviceSwapchainSupport(physicalDevice, details.SwapchainSupport);
  QueryDeviceExtensions(physicalDevice, details.Extensions);
}

void VulkanDevice::QueryDeviceQueues(VkPhysicalDevice physicalDevice,
  ...
  for (U32 i = 0; i < queues.Count; i++) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, i, _surface->GetSurface(),
                                         &queue.PresentKHR);
...
void VulkanDevice::QueryDeviceSwapchainSupport(VkPhysicalDevice physicalDevice,
                                               VulkanSwapchainSupport& support) {
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, _surface->GetSurface(),
                                            &support.Capabilities);

  U32 formatCount = 0;
  vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, _surface->GetSurface(), &formatCount,
                                       nullptr);
  if (formatCount != 0) {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, _surface->GetSurface(),
...
void VulkanDevice::QueryDeviceExtensions(VkPhysicalDevice physicalDevice,
...
const bool VulkanDevice::VerifyDeviceCapabilities(VulkanPhysicalDeviceDetails& details) {
...
const bool VulkanDevice::VerifyDeviceExtensions(
...
void VulkanDevice::DumpDeviceDetails(const VulkanPhysicalDeviceDetails& details) {
